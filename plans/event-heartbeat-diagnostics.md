...
All pipeline diagnostic logging (sequence tracking, gap detection, heartbeat trace info) is **opt-in** and controlled by a global setting. This prevents noisy logs during normal operation while ensuring we can capture full diagnostic data when issues are detected.
...
- Readable from all three layers:
  - **Frontend**: `settingsStoreClient.getOrDefault("diagnosticLogging", false)`
  - **Agent (Node.js)**: Check env var `MORT_DIAGNOSTIC_LOGGING=1` (set by runner on spawn if setting is enabled) — avoids agents needing to read settings files
  - **Rust AgentHub**: Check env var `MORT_DIAGNOSTIC_LOGGING=1` OR read from Tauri managed state (set on app init from settings file)

**Auto-enable on heartbeat drop detection**: When the frontend heartbeat monitor transitions a thread to `stale` status:
1. Automatically set `diagnosticLogging` to `true` via `settingsStoreClient.set("diagnosticLogging", true)`
2. Notify connected agents via a new `diagnostic:enable` relay message through the hub (so in-flight agents start logging without restart)
3. Set a Rust-side `AtomicBool` flag via Tauri command so the hub starts logging immediately
4. Log a single `logger.warn("[diagnostics] Auto-enabled diagnostic logging due to heartbeat staleness")` so it's visible in the log panel

**Manual toggle**: The diagnostic UI panel (Phase 5) exposes a toggle for `diagnosticLogging` so it can be enabled preemptively or disabled after investigation.

**What gets logged when enabled**:
- Every pipeline stage stamp on every message (see below)
- Sequence gap warnings with full context
- Heartbeat timing details (jitter, latency between agent timestamp and receipt)
- Write failure details on the agent side

**What always logs regardless of setting**:
- Heartbeat status transitions (`healthy → degraded → stale`) — these are rare, actionable events
- Sequence gap summary on agent completion (e.g., "Thread abc: 3 gaps detected, 12 events dropped total")
...
- **When `diagnosticLogging` is enabled**: `logger.debug` each message's seq and type (e.g., `[hub] sent seq=42 type=state`)
- **When disabled**: No per-message logging. Seq counter still increments (needed for gap detection) but nothing is logged
...
- **When `MORT_DIAGNOSTIC_LOGGING` is enabled**: `tracing::debug` every message's seq and type at both receive and emit points
...
- **When `diagnosticLogging` is enabled**: Log every message's full pipeline trail (all stages with timestamps, showing latency between hops)
...
- **When `diagnosticLogging` enabled**: Log heartbeat send with seq and timestamp
...
- On transition to `stale`:
  ...
  - **Auto-enable `diagnosticLogging`** if not already enabled (see Cross-Cutting section above)
...
- **Diagnostic logging toggle**: Switch to enable/disable `diagnosticLogging` setting. Shows current state (on/off) and whether it was auto-enabled by staleness detection. Label: "Pipeline Tracing" with description "Log every message at each pipeline stage (agent → hub → frontend)"
...
- `src/entities/settings/types.ts` — add `diagnosticLogging` to settings schema
...
3. **Diagnostic logging is opt-in by default**: Per-message pipeline trace logs are gated behind a `diagnosticLogging` global setting. This prevents log noise during normal operation. Status transitions (healthy → degraded → stale), gap warnings, and errors always log regardless of the setting — they're rare and actionable.
4. **Auto-enable diagnostics on heartbeat staleness**: When the frontend detects a stale heartbeat, it programmatically enables `diagnosticLogging`. This means full pipeline tracing kicks in exactly when the problem is happening, without requiring the developer to have pre-enabled it. The setting stays on after recovery so captured data can be reviewed.
...
13. **Three-tier logging approach**: (a) Always-on: status transitions, gaps, errors — low volume, high signal. (b) Opt-in diagnostic: full pipeline trace per message — high volume, used for investigation. (c) Agent-side `DEBUG` env var: existing pattern for agent-internal debug, orthogonal to pipeline diagnostics.
