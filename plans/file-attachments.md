...
- [x] Handle file drop events on the input area
- [x] Show image previews above input for detected image paths
- [x] Show image previews above sent user messages in the thread
- [x] Write tests
...
## Phase 3: Show Image Previews in Sent User Messages

**File: `src/components/thread/user-message.tsx`**

Like iMessage — when a sent message contains image paths, render the images above the text bubble. The user already sees the raw path text; images give them visual context for what was attached.

**Approach**: Reuse the same `extractImagePaths` helper from Phase 2. In `UserMessage`, extract image paths from the message content, render them above the text bubble, and strip the raw path lines from the displayed text.
...
export function UserMessage({ turn }: UserMessageProps) {
  ...
  return (
    <article role="article" aria-label="Your message" className="flex justify-end my-3">
      <div className="max-w-[80%] flex flex-col items-end gap-1">
        {/* Image previews above the bubble */}
        {imagePaths.length > 0 && (
        ...
        {/* Text bubble (only if there's non-image text) */}
        {textContent && (
          <div className={cn(
...
**`stripImagePaths` helper** (co-located with `extractImagePaths`):
```ts
...
This strips the raw `/path/to/image.png` lines from the displayed text since the images render visually. If the message was *only* image paths with no other text, the text bubble is omitted entirely — just the images show.

**Key details**:
- Images render at natural aspect ratio, capped at `max-h-48` (similar to iMessage)
- `rounded-xl` for the image corners to match the bubble aesthetic
- Multiple images flex-wrap in a row, right-aligned to match user bubble alignment
- Reuses `convertFileSrc` for the `src` URL and `extractImagePaths`/`isImagePath` from Phase 2
- No new state — purely derived from the message content string

## Phase 4: Tests

1. **Path extraction**: Unit test `extractImagePaths` and `stripImagePaths` — returns image paths, ignores non-image paths, handles empty/multiline content, strips correctly
2. **Drop handler**: Test that dropping files appends paths to input content
3. **Input preview strip**: Test that component renders thumbnails for image paths, renders nothing for non-image or empty content
4. **User message images**: Test that `UserMessage` renders images above the bubble for messages containing image paths, omits the text bubble when the message is image-only, and shows both images + text for mixed content
