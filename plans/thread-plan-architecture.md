...
interface ThreadMetadata {
  id: string                    // UUID
  status: ThreadStatus          // idle | running | completed | error | paused | cancelled
  workingDirectory: string      // Where agent runs
  ...
  changedFilePaths: string[]    // Files modified during execution
  isRead: boolean               // For inbox unread state
  pid?: number                  // Process ID when running
  worktreePath?: string         // Explicit worktree path
  createdAt: string
  updatedAt: string
}
```
...
interface PlanMetadata {
  id: string                    // UUID
  absolutePath: string          // Full path to the markdown file in repo
  relativePath: string          // Path relative to repo's plans directory
  isRead: boolean               // For inbox unread state
  parentPlanId?: string         // For nested plans (parent's UUID, derived from file hierarchy)
  createdAt: string
  updatedAt: string
}
```
...
Plan content lives at `absolutePath` in the repository (e.g., `{repo}/plans/auth-system/README.md`).
...
3. System detects new plan file, calls `ensurePlanExists({ absolutePath })`
...
- Plan metadata's `absolutePath` updated to reflect new location
...
1. **Detect existing data**: Check for `~/.mort/tasks/` directory
2. **Warn user**: "Mortician has a new architecture. Your existing tasks will not be migrated."
3. **Backup**: Move `~/.mort/tasks/` to `~/.mort/tasks-backup-{date}/`
4. **Fresh start**: Initialize new directory structure

Users lose:
- Existing task metadata
- Thread history (but git history remains)
- Task-plan associations

Users keep:
- Plan files (they're in the project, not ~/.mort)
