# Programmatic Agent Orchestration (mort-eval)
...
Allow agents to write and execute TypeScript orchestration code via a `mort-eval` Bash tool interception. The agent outputs TypeScript that uses an injected `mort` SDK object, enabling loops, `Promise.all` parallelism, and conditional logic — far more powerful than a shell CLI.
...
2. **No env vars or shell wrapper** — parent context is injected via the `mort` SDK object; `setupMortCommand()` can be removed
3. **PreToolUse:Bash hook interception** — detect `mort-eval` prefix, deny actual bash execution, run the code, return result as deny reason
4. **`mort.spawn()` returns a promise** — blocking by default, agent can `Promise.all` for parallelism
5. **Skills as invocation mechanism** — base system prompt does NOT mention mort-eval; a skill (e.g., `/orchestrate`) injects the instructions
...
PreToolUse:Bash hook fires
  → Detects "mort-eval" prefix
  → Extracts TypeScript code from heredoc
  → Creates AsyncFunction with `mort` as parameter
  → Executes with MortEvalSdk instance (has full parent context)
  → mort.spawn() creates child thread on disk, spawns child_process, waits for exit, reads result
...
| File | Purpose |
|------|---------|
| `agents/src/lib/mort-eval/eval-runner.ts` | `MortEvalRunner` class — parses code, creates AsyncFunction, executes with SDK |
| `agents/src/lib/mort-eval/mort-sdk.ts` | `MortEvalSdk` class — the injected `mort` object |
| `agents/src/lib/mort-eval/child-spawner.ts` | `ChildSpawner` class — spawns child process, waits, reads result |
| `agents/src/lib/mort-eval/types.ts` | Shared types (`SpawnOptions`, `SpawnResult`, `EvalContext`) |
| `agents/src/lib/mort-eval/index.ts` | Barrel export |
| `plugins/mort/skills/orchestrate/SKILL.md` | Skill prompt teaching the agent how to use mort-eval |

### Modified Files

| File | Change |
|------|--------|
| `agents/src/runners/shared.ts` | Add PreToolUse:Bash hook for mort-eval interception (~25 lines) |
| `agents/src/runner.ts` | Remove `setupMortCommand()` and its call |

## Phases

- [ ] Core eval infrastructure (eval-runner, code extraction, PreToolUse:Bash hook — no spawning yet, just `return 42` works)
- [ ] Child spawning (child-spawner, mort-sdk, wire into eval-runner)
- [ ] Skill prompt and cleanup (create `/orchestrate` skill, remove `setupMortCommand()`)
...
2. **Agent interpreting "deny" as failure** — prefix results with `mort-eval result:` so the agent recognizes success. Skill prompt teaches this.
...
- **Original plan**: Shell wrapper (`bin/mort`) + env vars for context discovery
...
- **Refinement**: No VM/sandbox, just `AsyncFunction` in the runner process. `mort.spawn` blocks but agent can `Promise.all`. API taught via skills, not system prompt.

Detailed implementation plan was produced by a child thread `4ebac685-3605-4e21-8e92-2b78a7b67692` ("mort eval typescript orchestration").
