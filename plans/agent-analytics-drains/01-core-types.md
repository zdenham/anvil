# 01 — Core Drain Event Types

## Summary

Create `core/types/drain-events.ts` with Zod schemas and TypeScript types for all drain events. This file is the shared contract between the TS agent (emission) and Rust layer (consumption). No runtime dependencies — pure type definitions.

## Phases

- [x] Create `core/types/drain-events.ts` with all schemas
- [x] Export from `core/types/index.ts` (if barrel file exists, otherwise skip)

<!-- IMPORTANT: Mark phases complete with [x] as you finish them. Update this file immediately after completing each phase - do not batch updates. -->

---

## File: `core/types/drain-events.ts`

### Structure

Follow the pattern in `core/types/events.ts` — Zod schemas as source of truth, inferred TS types, const object for event names.
...
export type DrainEventNameType = (typeof DrainEventName)[keyof typeof DrainEventName];
...
export const ToolFailedPropsSchema = z.object({
  ...
  durationMs: z.number(),
  error: z.string(),
  errorType: z.enum(["permission_denied", "execution_error", "timeout", "unknown"]),
});
...
export interface DrainEventPayloads {
  ...
  [DrainEventName.THREAD_LIFECYCLE]: z.infer<typeof ThreadLifecyclePropsSchema>;
  [DrainEventName.CONTEXT_PRESSURE]: z.infer<typeof ContextPressurePropsSchema>;
  [DrainEventName.SUBAGENT_SPAWNED]: z.infer<typeof SubagentSpawnedPropsSchema>;
  [DrainEventName.SUBAGENT_COMPLETED]: z.infer<typeof SubagentCompletedPropsSchema>;
  [DrainEventName.PERMISSION_DECIDED]: z.infer<typeof PermissionDecidedPropsSchema>;
  [DrainEventName.CONTEXT_COMPACTED]: z.infer<typeof ContextCompactedPropsSchema>;
...
- Follow `core/types/events.ts` patterns: Zod schemas, const event names, `z.infer<>` types
- Properties use flat key-value (no nesting) — matches EAV storage model
- All property values are `string | number | boolean` — no objects, no arrays (encode as JSON strings)
- Optional fields use `.optional()` — Rust layer skips undefined properties
