...
- **Headless styling** — Tiptap is unstyled by default. We apply Tailwind `prose prose-invert prose-sm` which inherits the app's typography config (Inter for body, Atkinson Hyperlegible Mono for headings/code). No custom font overrides needed — just use the existing prose classes.
...
- [x] Install Tiptap dependencies and create `TiptapEditor` component
- [x] Wire markdown serialization (load from string, serialize on change)
- [x] Integrate into `file-content.tsx` rendered mode for markdown files
- [x] Add a minimal floating toolbar for formatting
- [x] Style the editor to match the app's prose theme
...
Apply styles to the Tiptap `.ProseMirror` container using the existing Tailwind typography classes. The `prose` plugin config in `tailwind.config.js` already defines Inter for body text and Atkinson Hyperlegible Mono for headings/code — Tiptap inherits all of this automatically:
