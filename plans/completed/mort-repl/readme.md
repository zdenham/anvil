# mort-repl: Programmatic Agent Orchestration

Allow agents to write and execute TypeScript orchestration code via a `mort-repl` Bash tool interception. The agent outputs code that uses an injected `mort` SDK object, enabling loops, `Promise.all` parallelism, and conditional logic for recursive agent spawning.
...
2. **PreToolUse:Bash hook interception** — detect `mort-repl` prefix, deny actual bash execution, run the code, return result as deny reason
3. **`mort.spawn()` returns a promise** — blocking by default, agent can `Promise.all` for parallelism
4. **Skills as invocation mechanism** — base system prompt does NOT mention mort-repl; a skill (`/orchestrate`) injects the instructions + API reference
...
Agent calls Bash tool:
  command: mort-repl <<'MORT_REPL'
  const [a, b] = await Promise.all([
    mort.spawn({ prompt: 'fix auth tests' }),
    mort.spawn({ prompt: 'fix api tests' }),
  ]);
  return { auth: a, api: b };
  MORT_REPL
        ↓
PreToolUse:Bash hook fires
  → Detects "mort-repl" prefix
  → Extracts code from heredoc (or quoted string)
  → ts.transpileModule(code) → strips types → plain JS
  → Creates AsyncFunction with `mort` as parameter
  → Executes with MortReplSdk instance (has full parent context)
  → mort.spawn() creates child thread on disk, spawns child_process, waits for exit, reads last assistant message
  → Returns deny decision with formatted result as reason
        ↓
Agent sees the result string as tool output
```

The **deny mechanism** is how results get back to the agent. The SDK surfaces deny reasons as tool error messages, which the agent reads and acts on. This is the same pattern as `mort-resolve-comment`.

## `mort` SDK Shape

```typescript
// Available as `mort` in the REPL context

// Spawn a child agent and wait for completion
// Returns the child's last assistant message (string)
const result: string = await mort.spawn({
...
mort.context // { threadId, repoId, worktreeId, workingDir, permissionModeId }
```
...
3. Spawns `child_process.spawn('node', ['agents/dist/runner.js', '--thread-id', childId, '--repo-id', repoId, ...])` — same CLI args as `agent-service.ts:spawnSimpleAgent()`, same runner entry point, same env vars (inherits `ANTHROPIC_API_KEY`, `MORT_DATA_DIR`, etc.)
...
- `parentToolUseId` set to the Bash call's `tool_use_id` → allows UI to render spawned agents inline with the mort-repl call
...
| File | Purpose |
|------|---------|
| `agents/src/lib/mort-repl/repl-runner.ts` | `MortReplRunner` — parses code, esbuild-transforms TS→JS, creates AsyncFunction, executes with SDK |
| `agents/src/lib/mort-repl/mort-sdk.ts` | `MortReplSdk` — the injected `mort` object with `spawn()`, `log()`, `context` |
| `agents/src/lib/mort-repl/child-spawner.ts` | `ChildSpawner` — thin wrapper: creates thread on disk, spawns existing `runner.js` via `child_process`, tracks PIDs, waits for exit, reads result. No custom agent loop logic. |
| `agents/src/lib/mort-repl/types.ts` | Shared types (`SpawnOptions`, `SpawnResult`, `ReplContext`) |
| `agents/src/lib/mort-repl/index.ts` | Barrel export |
| `agents/src/hooks/repl-hook.ts` | PreToolUse hook — detects `mort-repl`, delegates to `MortReplRunner` |
| `plugins/mort/skills/orchestrate/SKILL.md` | Skill prompt teaching the agent how to use mort-repl + API reference |
...
- [x] Child spawning — child-spawner, mort-sdk with `spawn()` returning last assistant message, wire into repl-runner, emit thread:created
...
2. **Agent interpreting "deny" as failure** — prefix results with `mort-repl result:` so the agent recognizes success. Skill prompt teaches this.
