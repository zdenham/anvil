...
6. **Three modes, not four:** Plan (default) → Implement → Approve. No "Locked" mode.
...
10. **Approve mode:** Shows a diff preview for Write/Edit calls. The user approves or denies before the edit is applied to disk (the hook blocks, file is unchanged until approval).
11. **Bash gating is out of scope:** Approve mode does NOT prompt for Bash commands in this plan. Future work will add Bash gating, but it's explicitly excluded here to keep scope manageable. Plan and Implement modes auto-allow Bash.
...
15. **Cycle order:** Plan → Implement → Approve → Plan.
...
- [x] Implement PreToolUse permission hook with event-based approval flow
- [x] Add pinned permission request block above chat input (Claude Code-style)
- [x] Add "needs input" yellow dot variant for threads awaiting user action
- [x] Add below-input status bar (mode selector + context meter)
- [x] Notify agent of mode changes via streamInput message injection
- [x] Add tests for permission evaluation and approval flow
...
type PermissionModeId = "plan" | "implement" | "approve";
...
const APPROVE_MODE: PermissionMode = {
  id: "approve",
  name: "Approve",
...
**Cycle order:** Plan → Implement → Approve → Plan
...
In **Approve mode**, the block shows a diff preview (old content vs new content) for Write/Edit tool calls:
...
- For Write/Edit in Approve mode: shows diff preview (reading current file content to generate the diff)
...
- **Shift+Tab** cycles through modes: Plan → Implement → Approve → Plan (only when input is focused)
...
| Approve | `text-yellow-400` | `Approve` |
...
[EventName.PERMISSION_MODE_CHANGED]: {
  threadId: string;
  modeId: PermissionModeId; // "plan" | "implement" | "approve"
};
```
...
- `[System] Permission mode changed to "Approve". File edits (Write/Edit) now require user approval before being applied.`
...
- Integration tests for the PreToolUse hook:
  ...
  - Ask-and-wait flow in Approve mode (mock frontend response)
...
| `src/components/permission/permission-request-block.tsx` | **New** — Pinned permission block above input (diff preview for Approve, keyboard nav, auto-focus) |
