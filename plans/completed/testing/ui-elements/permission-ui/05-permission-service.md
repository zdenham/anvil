# Sub-Plan 05: Permission Service

## Scope

Create the frontend service that handles permission responses, coordinating between the store, agent service, and event bus.

## Dependencies

- **02-zustand-store.md** - Requires `usePermissionStore`
- **04-agent-service-stdin.md** - Requires `sendPermissionResponse`

## Files to Create

### `src/entities/permissions/service.ts` (~60 lines)
...
```bash
pnpm tsc --noEmit
pnpm test -- src/entities/permissions/service
```
...
- Follows entity-stores pattern: service is the only writer to store
- Uses optimistic updates with rollback on failure
- Emits events for debugging and potential future features
- Batch operations (approveAll/denyAll) for convenience
