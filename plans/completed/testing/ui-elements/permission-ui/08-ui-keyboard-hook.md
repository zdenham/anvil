# Sub-Plan 08: Permission Keyboard Hook

## Scope

Create the keyboard shortcut hook that handles approve/deny actions and navigation for both modal and inline display modes.
...
### `src/components/permission/use-permission-keyboard.ts` (~80 lines)
...
Include tests from main plan's "Test 5: Keyboard Hook" section.

## Verification

```bash
pnpm tsc --noEmit
pnpm test -- src/components/permission/use-permission-keyboard
```
...
- Follows React rules: useEffect for event subscription is legitimate
- Ignores keystrokes when focus is in input fields
- Different keybindings for modal vs inline modes
- Vim-style j/k navigation for inline mode
