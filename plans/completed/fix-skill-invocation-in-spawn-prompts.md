...
When skills recursively invoke other skills via `anvil.spawn()`, the slash command appears **mid-sentence** in the prompt — e.g., `"Use /anvil:decompose to execute: ..."`. Claude Code only auto-expands slash commands into `<command-name>` tags when the command is at the **start** of the user message. Mid-sentence references require an extra Skill tool call round-trip, wasting tokens and adding latency.
...
Both files exist in two locations (plugin source + installed copy). Edit the plugin source; the installed copy at `~/.anvil/skills/` should be synced separately.

### 1. `plugins/anvil/skills/decompose/SKILL.md`

**Line 92** — the example `anvil.spawn()` prompt:
...
### 2. `plugins/anvil/skills/breadcrumb-loop/SKILL.md`

**Line 33** — the `anvil.spawn()` prompt is already correct:
...
- [x] Sync changes to installed copies at `~/.anvil/skills/`
