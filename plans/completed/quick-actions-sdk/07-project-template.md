...
Create the default project template that ships with Mort and gets copied to `~/.mort/quick-actions/` on first launch. This includes all configuration files, the build script, and example actions.

## Files to Create

All files in `core/sdk/template/` that will be copied to `~/.mort/quick-actions/`:
...
Note: `@mort/sdk` is NOT listed as a dependency because it's pre-installed in `node_modules/`.
...
These are generated by running `npm run build` in the template directory during Mort's build process.

### SDK Package (pre-installed)

The template includes a minimal `node_modules/@mort/sdk/` directory:

- `core/sdk/template/node_modules/@mort/sdk/package.json`
- `core/sdk/template/node_modules/@mort/sdk/index.d.ts`
- `core/sdk/template/node_modules/@mort/sdk/index.js`
...
- [ ] @mort/sdk package pre-installed in node_modules
...
- **#4 SDK Distribution**: Types are in `node_modules/@mort/sdk/index.d.ts`, actual SDK injected at runtime
- **#5 Runtime Dependency**: devDependencies use tsx for building; output is vanilla JS requiring only Node.js
- **#10 & #33 SDK Communication**: Example actions use SDK methods like `sdk.threads.archive()` which emit events - Mort handles actual writes
- **#14 Action IDs**: Manifest `slug` field is human-readable; Mort assigns UUIDs when registering
- **#16 Context Scope**: Actions specify contexts array; 'all' is equivalent to ['thread', 'plan', 'empty']
- **#22 SDK Types Distribution**: Only .d.ts shipped; runtime implementation injected by Mort's runner
...
ls -la node_modules/@mort/sdk/package.json node_modules/@mort/sdk/index.d.ts node_modules/@mort/sdk/index.js
```
...
- @mort/sdk types are properly referenced
...
import { defineAction, QuickActionDefinition, QuickActionExecutionContext, MortSDK } from '@mort/sdk';
...
const testAction: QuickActionDefinition = {
  ...
  title: 'Test',
  contexts: ['thread'],
  execute: async (context: QuickActionExecutionContext, sdk: MortSDK) => {
...
if (pkg.dependencies && pkg.dependencies['@mort/sdk']) throw new Error('@mort/sdk should not be in dependencies');
...
```bash
# Create temp directory simulating ~/.mort/quick-actions/
TEMP_QA=$(mktemp -d)
