...
1. **Build-time baking**: `MORT_APP_SUFFIX` and hotkeys are baked into the binary via `build.rs`
...
|----------|---------|-----------|---------|
| `MORT_APP_SUFFIX` | App identifier/name suffix | Build-time (baked) | _(none)_ |
| `MORT_VITE_PORT` | Vite dev server port | Build-time | `1420` |
| `MORT_SPOTLIGHT_HOTKEY` | Spotlight hotkey | Build-time (baked) | `Command+Space` |
| `MORT_CLIPBOARD_HOTKEY` | Clipboard hotkey | Build-time (baked) | `Command+Option+C` |
| `MORT_DATA_DIR` | Data directory override | Runtime (optional) | Derived from suffix |
| `MORT_CONFIG_DIR` | Config directory override | Runtime (optional) | Derived from suffix |

## Phase 1: Build Scripts & Configuration

- [ ] Create shell script presets
  - `scripts/env-presets/dev.sh` - exports `MORT_APP_SUFFIX=dev`, `MORT_VITE_PORT=1421`, hotkeys

- [ ] Create build wrapper scripts
  - `scripts/dev-mort.sh` - sources preset, runs `pnpm dev:run`
  - `scripts/build-mort.sh` - builds stable app (no preset)
  - Make executable: `chmod +x scripts/*.sh`

- [ ] Create `src-tauri/tauri.conf.dev.json` overlay
  - Set `identifier` to `com.getmort.app.dev`
  - Set `productName` to `Mort Dev`
  - Set `devUrl` to `http://localhost:1421`
  - Reference `icons-dev/` paths

- [ ] Update `vite.config.ts`
  - Read `MORT_VITE_PORT` env var (default 1420)
  - Read `MORT_APP_SUFFIX` env var
  - Define `__MORT_APP_SUFFIX__` global
...
- [ ] Update `src-tauri/build.rs`
  - Bake `MORT_APP_SUFFIX` via `cargo:rustc-env`
  - Bake `MORT_SPOTLIGHT_HOTKEY` via `cargo:rustc-env`
  - Bake `MORT_CLIPBOARD_HOTKEY` via `cargo:rustc-env`

- [ ] Create `src-tauri/src/build_info.rs`
  - `APP_SUFFIX: &str = env!("MORT_APP_SUFFIX")`
  - `DEFAULT_SPOTLIGHT_HOTKEY: &str = env!("MORT_SPOTLIGHT_HOTKEY")`
  - `DEFAULT_CLIPBOARD_HOTKEY: &str = env!("MORT_CLIPBOARD_HOTKEY")`
...
- [ ] Create `src-tauri/src/paths.rs`
  ...
  - `default_data_dir()` → `~/.mort` or `~/.mort-{suffix}`
  - `default_config_dir()` → `mortician` or `mortician-{suffix}`
...
- [ ] Install to /Applications: `cp -r src-tauri/target/release/bundle/macos/Mort.app /Applications/`
- [ ] Launch Mort.app from Finder (stable, Cmd+Space hotkey)
...
| `scripts/dev-mort.sh` | Dev wrapper script |
| `scripts/build-mort.sh` | Build wrapper script |
...
| `vite.config.ts` | Read `MORT_VITE_PORT`, `MORT_APP_SUFFIX` env vars |
...
| `src-tauri/src/mort_commands.rs` | Use `paths` module, add `get_paths_info` command |
