...
describe("MessageHandler", () => {
  ...
  describe("handleUser", () => {
    ...
    it("ignores synthetic/initial user messages", async () => {
      // User messages without isSynthetic (undefined) are treated as non-tool
      const msg: SDKUserMessage = {
        type: "user",
        message: {
          role: "user",
      ...
      expect(logger.debug).toHaveBeenCalledWith(
        "[MessageHandler] Ignoring non-tool user message"
      );
    });

    it("ignores non-tool user messages (isSynthetic: false)", async () => {
      // Queued message handling now lives in SocketMessageStream / QueuedAckManager
      const msg: SDKUserMessage = {
        type: "user",
        message: {
          role: "user",
  ...
  describe("ackManager delegation", () => {
    it("calls ackManager.onAssistantTurn() on assistant messages with usage", async () => {
      const mockAckManager = {
        onAssistantTurn: vi.fn(),
        drainNacks: vi.fn(),
        register: vi.fn(),
        get size() { return 0; },
      };
      const handlerWithAck = new MessageHandler(undefined, undefined, undefined, undefined, mockAckManager as any);
      ...
      await handlerWithAck.handle(msg);
      expect(mockAckManager.onAssistantTurn).toHaveBeenCalledOnce();
    });

    it("calls ackManager.drainNacks() on result message", async () => {
      const mockAckManager = {
        onAssistantTurn: vi.fn(),
        drainNacks: vi.fn(),
        register: vi.fn(),
      ...
      await handlerWithAck.handle(msg);
      expect(mockAckManager.drainNacks).toHaveBeenCalledOnce();
    });

    it("works without ackManager (no-op)", async () => {
      // Default handler has no ackManager — should not throw
      const msg: SDKAssistantMessage = {
        type: "assistant",
        message: {
          id: "msg_123",
