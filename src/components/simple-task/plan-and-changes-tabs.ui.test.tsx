 ...
 *
 * Tests that plans and file changes render correctly in the simple task view.
 * Uses test data based on a real task that created plans/goodbye-world.md.
 */
...
// Real test data matching the task at /Users/zac/.mort-dev/tasks/9e2e36f0-0689-4e4b-90cf-a996558aa7fd
const TASK_ID = "9e2e36f0-0689-4e4b-90cf-a996558aa7fd";
...
describe("PlanTab", () => {
  ...
  describe("when plan exists in store", () => {
    it("renders plan content when planId and plan are present", async () => {
      ...
      expect(screen.getByText(/plans\/goodbye-world\.md/)).toBeInTheDocument();
    });
...
describe("ChangesTab", () => {
  ...
  describe("when threadState is undefined", () => {
    ...
    it("shows loading state initially when threadState has not been received yet", () => {
      ...
      // Should show loading state while waiting for initial state
      expect(screen.getByText("Generating diff...")).toBeInTheDocument();
    });
  });
