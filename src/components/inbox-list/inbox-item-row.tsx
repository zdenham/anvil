 ...
 */
function ArchiveButton({ onArchive }: ArchiveButtonProps) {
  ...
  return (
    ...
    >
      {confirming ? "confirm" : <Archive size={14} />}
    </button>
  );
}
 ...
 */
export function InboxItemRow({ item, onSelect, isSelected, onArchive }: InboxItemRowProps) {
  ...
  const handleArchive = useCallback(async () => {
    ...
    onArchive?.();
  }, [item.type, item.data.id, onArchive]);
