...
mod baked {
    ...
    pub const WS_PORT: &str = env!("ANVIL_WS_PORT");
}
...
pub fn app_suffix() -> &'static str {
    APP_SUFFIX_VAL.get_or_init(|| resolve("ANVIL_APP_SUFFIX", baked::APP_SUFFIX))
}

/// Default spotlight hotkey
pub fn default_spotlight_hotkey() -> &'static str {
    SPOTLIGHT_HOTKEY_VAL.get_or_init(|| resolve("ANVIL_SPOTLIGHT_HOTKEY", baked::DEFAULT_SPOTLIGHT_HOTKEY))
}

/// Default clipboard hotkey
pub fn default_clipboard_hotkey() -> &'static str {
    CLIPBOARD_HOTKEY_VAL.get_or_init(|| resolve("ANVIL_CLIPBOARD_HOTKEY", baked::DEFAULT_CLIPBOARD_HOTKEY))
}

/// WebSocket server port (default "9600", dev uses "9601")
pub fn ws_port() -> &'static str {
    WS_PORT_VAL.get_or_init(|| resolve("ANVIL_WS_PORT", baked::WS_PORT))
}
